...
- `createNamespace`, `verifyPat`, `publish` and `delete` now send the personal access token primarily via an `Authorization: Bearer` header (or, when `OVSX_USERNAME`/`OVSX_PASSWORD` are set, an `X-OpenVSX-Token` header, since `Authorization` is then already in use for Basic auth to a fronting proxy), moving away from a `token` query parameter that gets recorded in server and reverse-proxy access logs. This CLI still sends the token as a query parameter as well, for compatibility with registries that predate header support - so it is still recorded in logs until that's dropped in a later release, once such registries can be assumed to have moved on ([#1344](https://github.com/eclipse-openvsx/openvsx/issues/1344))
