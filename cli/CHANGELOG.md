...
- `createNamespace`, `verifyPat`, `publish` and `delete` now send the personal access token via the `X-OpenVSX-Token` header instead of a `token` query parameter, so it no longer needs to be recorded in server and reverse-proxy access logs. Registries running an older version still accept the token via the query parameter ([#1344](https://github.com/eclipse-openvsx/openvsx/issues/1344))
