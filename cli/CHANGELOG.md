...
- `createNamespace`, `verifyPat`, `publish` and `delete` now send the personal access token via an `Authorization: Bearer` header (or, when `OVSX_USERNAME`/`OVSX_PASSWORD` are set, an `OpenVSX-Token` header, since `Authorization` is then already in use for Basic auth to a fronting proxy) instead of relying solely on a `token` query parameter, so it no longer needs to be recorded in server and reverse-proxy access logs. The query parameter is still sent alongside the header for compatibility with registries that predate this change ([#1344](https://github.com/eclipse-openvsx/openvsx/issues/1344))
